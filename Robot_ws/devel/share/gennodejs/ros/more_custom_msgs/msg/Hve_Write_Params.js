...
class Hve_Write_Params {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.parameters.forEach((val) => {
      bufferOffset = Hve_Conf_Param.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new Hve_Write_Params(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.parameters[i] = Hve_Conf_Param.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.parameters.forEach((val) => {
      length += Hve_Conf_Param.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new Hve_Write_Params(null);
    ...
    if (msg.parameters !== undefined) {
      ...
      for (let i = 0; i < resolved.parameters.length; ++i) {
        resolved.parameters[i] = Hve_Conf_Param.Resolve(msg.parameters[i]);
      }
    }
    else {
      resolved.parameters = []
...
module.exports = Hve_Write_Params;
